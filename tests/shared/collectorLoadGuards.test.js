...
function withTmpHome(prepare) {
  const tmp = fs.mkdtempSync(path.join(fs.realpathSync.native(os.tmpdir()), 'personal-monitor-home-'));
...
function withAliasedTmpHome(prepare) {
  const base = fs.mkdtempSync(path.join(fs.realpathSync.native(os.tmpdir()), 'personal-monitor-alias-'));
