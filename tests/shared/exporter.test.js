...
test('renderExportJson has the documented shape', () => {
  const json = JSON.parse(renderExportJson({ periods: PERIODS, history: HISTORY, meta: { generatedAt: 'X', app: { name: 'personal-monitor', version: '1.2.3' } } }));
  assert.equal(json.generatedAt, 'X');
  assert.deepEqual(json.app, { name: 'personal-monitor', version: '1.2.3' });
...
test('exportFileSet omits daily.csv when history has no daily rows', () => {
  ...
  assert.deepEqual(withDaily.map((f) => f.name), ['personal-monitor-export.json', 'personal-monitor-snapshot.csv', 'personal-monitor-daily.csv']);

  const noDaily = exportFileSet({ periods: PERIODS, history: { daily: [], monthly: [] } });
  assert.deepEqual(noDaily.map((f) => f.name), ['personal-monitor-export.json', 'personal-monitor-snapshot.csv']);
});
