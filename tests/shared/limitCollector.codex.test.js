...
test('fetchCodexLimits returns one provider per managed Codex account', async () => {
  ...
  const providers = await fetchCodexLimits({
    codexManagedAccounts: [
      { id: 'one', email: 'one@example.com', homePath: '/tmp/personal-monitor-codex/one' },
      { id: 'two', email: 'two@example.com', homePath: '/tmp/personal-monitor-codex/two' }
    ]
  }, {
  ...
  assert.deepEqual(seenHomes, ['/tmp/personal-monitor-codex/one', '/tmp/personal-monitor-codex/two']);
    ...
    codexManagedAccounts: [
      {
        ...
        homePath: '/tmp/personal-monitor-codex/personal'
      },
      {
        id: 'team',
        ...
        workspaceLabel: 'Team',
        homePath: '/tmp/personal-monitor-codex/team'
      }
    ]
  }, {
    now: () => Date.parse('2026-06-01T00:00:00Z'),
  ...
  const accounts = [
    {
      ...
      workspaceAccountId: 'workspace-team',
      homePath: '/tmp/personal-monitor-codex/one',
      authPath: '/tmp/personal-monitor-codex/one/auth.json'
    },
    {
      id: 'two',
      email: 'two@example.com',
      workspaceAccountId: 'workspace-team',
      homePath: '/tmp/personal-monitor-codex/two',
      authPath: '/tmp/personal-monitor-codex/two/auth.json'
    }
  ];
  const providers = await fetchCodexLimits({
    includeLiveCodexAccount: false,
...
test('fetchCodexLimits can refresh only the requested managed Codex account', async () => {
  ...
  const providers = await fetchCodexLimits({
    ...
    codexManagedAccounts: [
      { id: 'other', accountKey: 'sha256:other', email: 'other@example.com', homePath: '/tmp/personal-monitor-codex/other' },
      { id: 'target', accountKey: 'sha256:target', email: 'target@example.com', homePath: '/tmp/personal-monitor-codex/target' }
    ]
  }, {
    now: () => Date.parse('2026-06-01T00:00:00Z'),
    env: { PATH: '/usr/bin' },
  ...
  assert.deepEqual(seenHomes, ['/tmp/personal-monitor-codex/target']);
...
test('fetchCodexLimits skips disabled managed Codex accounts', async () => {
  ...
  const providers = await fetchCodexLimits({
    codexManagedAccounts: [
      { id: 'one', email: 'one@example.com', homePath: '/tmp/personal-monitor-codex/one', enabled: true },
      { id: 'two', email: 'two@example.com', homePath: '/tmp/personal-monitor-codex/two', enabled: false }
    ]
  }, {
    now: () => Date.parse('2026-06-01T00:00:00Z'),
    env: { PATH: '/usr/bin' },
  ...
  assert.deepEqual(seenHomes, ['/tmp/personal-monitor-codex/one']);
  assert.equal(providers.length, 1);
...
test('fetchCodexLimits keeps the live system account visible alongside managed accounts', async () => {
  ...
  assert.deepEqual(seenHomes, ['<live>', '/tmp/personal-monitor-codex/two']);
...
test('fetchCodexLimits does not show the live account twice when it is also managed', async () => {
  const providers = await fetchCodexLimits({
    codexManagedAccounts: [
      { id: 'a', email: 'a@example.com', homePath: '/tmp/personal-monitor-codex/a' }
    ]
  }, {
    now: () => Date.parse('2026-06-01T00:00:00Z'),
    env: { PATH: '/usr/bin' },
...
test('fetchCodexLimits dedups the live account against the same managed account by account id (no email needed)', async () => {
  ...
  const providers = await fetchCodexLimits({
    codexManagedAccounts: [
      { id: 'm', email: '', accountKey: sharedKey, homePath: '/tmp/personal-monitor-codex/m' }
    ]
  }, {
    now: () => Date.parse('2026-06-01T00:00:00Z'),
    env: { PATH: '/usr/bin' },
...
test('fetchCodexLimits fills the live account email from auth.json when the RPC omits it', async () => {
  ...
  const providers = await fetchCodexLimits({
    codexManagedAccounts: [
      { id: 'm', email: 'managed@example.com', accountKey: 'sha256:managed', homePath: '/tmp/personal-monitor-codex/m' }
    ]
  }, {
    now: () => Date.parse('2026-06-01T00:00:00Z'),
    env: { PATH: '/usr/bin' },
...
test('fetchCodexLimits augments reset credits expiry from the Codex OAuth endpoint', async () => {
  ...
  const providers = await fetchCodexLimits({}, {
    now: () => Date.parse('2026-06-30T00:00:00Z'),
    env: { PATH: '/usr/bin', CODEX_HOME: '/tmp/personal-monitor-codex/live' },
    codexAuthPath: '/tmp/personal-monitor-codex/live/auth.json',
    codexCommand: 'codex',
    readFileSync: (file) => {
