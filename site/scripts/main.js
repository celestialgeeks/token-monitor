...
function setupDashboard() {
  ...
  renderHeatmap();
  window.addEventListener("personal-monitor-languagechange", renderHeatmap);
  renderChart();
...
function setupGitHubStars() {
  ...
  var cacheKey = "personal-monitor-github-stars";
  ...
  window.fetch("https://api.github.com/repos/Javis603/personal-monitor", {
...
function setupSmartDownloads() {
  ...
  var releasePage = "https://github.com/Javis603/personal-monitor/releases/latest";
  var apiUrl = "https://api.github.com/repos/Javis603/personal-monitor/releases/latest";
  var cacheKey = "personal-monitor-latest-release-v1";
  ...
  function assetUrlFor(key) {
    var matches = assets.filter(function (asset) {
      ...
      if (key === "windows") return /^Personal-Monitor-Setup-.*\.exe$/i.test(name);
