...
> 屬於 **[Personal Monitor](https://github.com/Javis603/personal-monitor)** 專案。這個目錄只是 Cloudflare Worker hub；桌面小工具、無頭 agent 和完整文件都在主倉庫。一鍵部署會建立一份獨立副本，不會自動更新，所以請回主倉庫查看新版本。
...
2. `x-personal-monitor-secret: <secret>`：無法設定 `Authorization` 的客戶端的後備方案。
