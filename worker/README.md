...
# Personal Monitor Hub — Cloudflare Worker

> Part of **[Personal Monitor](https://github.com/Javis603/personal-monitor)**. This directory is just the Cloudflare Worker hub; the desktop widget, headless agent, and full docs live in the main repo. A one-click deploy creates a standalone copy that won't auto-update, so check the main repo for new versions.
...
```
https://personal-monitor-hub.<your-subdomain>.workers.dev
```
...
- Hub URL: `https://personal-monitor-hub.<your-subdomain>.workers.dev`
...
```env
TOKEN_MONITOR_HUB_URL=https://personal-monitor-hub.<your-subdomain>.workers.dev
...
```js
const HUB = 'https://personal-monitor-hub.<your-subdomain>.workers.dev';
...
```js
const req = new Request('https://personal-monitor-hub.<your-subdomain>.workers.dev/api/stats');
...
2. `x-personal-monitor-secret: <secret>` — fallback for clients that cannot set
   `Authorization`.
