...
Personal Monitor can export your usage data in **tool-agnostic** formats so you can
...
- **Export data…** — pick a folder; Personal Monitor writes the file set below into
  it once, right now.
...
| File | What it is |
|---|---|
| `personal-monitor-export.json` | Complete, lossless snapshot + history in one JSON object |
| `personal-monitor-snapshot.csv` | Current totals (today / month / all-time), one row per tool and per model |
| `personal-monitor-daily.csv` | Daily time-series **history**, one row per day × tool — spans your whole tracked history, not just today (only written when trend history has data) |
...
`personal-monitor-snapshot.csv` under `period,today`, not here.
...
### `personal-monitor-export.json`

```json
{
  "generatedAt": "2026-07-03T14:30:00.000Z",
  "app": { "name": "personal-monitor", "version": "0.19.0" },
...
````markdown
```dataviewjs
const raw = await app.vault.adapter.read("TokenMonitor/personal-monitor-export.json");
...
Prefer a code-free table? Import `personal-monitor-daily.csv` with a CSV/table
plugin instead.

### Excel / Google Sheets / Numbers

Open `personal-monitor-snapshot.csv` or `personal-monitor-daily.csv` directly. Both are
tidy (long) tables, so they pivot cleanly.

### Grafana / dashboards

Use `personal-monitor-export.json` (or the CSVs) as a JSON/CSV file data source. For
