...
Routed Monitoring can export your usage data in **tool-agnostic** formats so you can
...
- **Export data…** — pick a folder; Routed Monitoring writes the file set below into
  it once, right now.
...
| File | What it is |
|---|---|
| `routed-monitoring-export.json` | Complete, lossless snapshot + history in one JSON object |
| `routed-monitoring-snapshot.csv` | Current totals (today / month / all-time), one row per tool and per model |
| `routed-monitoring-daily.csv` | Daily time-series **history**, one row per day × tool — spans your whole tracked history, not just today (only written when trend history has data) |
...
### `routed-monitoring-snapshot.csv`
...
`routed-monitoring-snapshot.csv` under `period,today`, not here.
...
### `routed-monitoring-export.json`

```json
{
  "generatedAt": "2026-07-03T14:30:00.000Z",
  "app": { "name": "routed-monitoring", "version": "0.19.0" },
...
````markdown
```dataviewjs
const raw = await app.vault.adapter.read("TokenMonitor/routed-monitoring-export.json");
...
Prefer a code-free table? Import `routed-monitoring-daily.csv` with a CSV/table
plugin instead.

### Excel / Google Sheets / Numbers

Open `routed-monitoring-snapshot.csv` or `routed-monitoring-daily.csv` directly. Both are
tidy (long) tables, so they pivot cleanly.

### Grafana / dashboards

Use `routed-monitoring-export.json` (or the CSVs) as a JSON/CSV file data source. For
