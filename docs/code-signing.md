...
The current release workflow Authenticode-signs Personal Monitor's Windows application executable, installer (`Personal-Monitor-Setup-*.exe`), and portable build (`Personal-Monitor-*.exe`). The expected publisher is **SignPath Foundation**.
...
```powershell
Get-AuthenticodeSignature ".\Personal-Monitor-Setup-<version>.exe", ".\Personal-Monitor-<version>.exe" | Format-List Path, Status, SignerCertificate, TimeStamperCertificate
```

A genuine Personal Monitor release should show `Valid` status, SignPath Foundation as the signer, and timestamp information.

After installation, the same check can be run against `Personal Monitor.exe` in the application's installation directory.
...
- Privacy policy: [Personal Monitor privacy policy](privacy.md)
