...
(function exposeTrayComposer(root, factory) {
  ...
  function createTrayComposer(options) {
    ...
    function styleTitle(style) {
      const fallback = {
        appIcon: 'Routed Monitoring icon',
    ...
    function barIconEditor(item) {
      ...
      const choices = [{
        value: 'app',
        label: l('trayComposer.icon.app', 'Routed Monitoring'),
        icon: app?.icon
      }, choiceForRow(0)];
    ...
    function renderItemPopover(id) {
      ...
      if (item.type === 'icon') {
        const iconTypes = [
          { value: 'app', label: l('trayComposer.icon.app', 'Routed Monitoring'), icon: providerChoices('auto').find((choice) => choice.value === 'auto')?.icon },
