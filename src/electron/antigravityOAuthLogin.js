...
function callbackPage(ok) {
  ...
  const detail = ok
    ? 'Return to Routed Monitoring to finish connecting this account.'
    : 'Return to Routed Monitoring for details.';
...
async function runAntigravityOAuthLogin(options = {}) {
  ...
  if (!client) {
    throw loginError(
      ...
      'Antigravity OAuth is unavailable in this build. Update Routed Monitoring or configure ANTIGRAVITY_OAUTH_CLIENT_ID and ANTIGRAVITY_OAUTH_CLIENT_SECRET.'
    );
  }
