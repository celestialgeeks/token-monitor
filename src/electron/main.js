...
const APP_NAME = 'Personal Monitor';
...
function reportCredentialStorageError(context, error) {
  ...
  try {
    dialog.showErrorBox(
      ...
      `Personal Monitor could not safely access credentials.json (${context}). The save was stopped and previous data was restored where possible. Check the file's JSON and permissions, then restart the app.\n\n${detail}`
    );
  } catch (_) {}
}
...
function updateTrayDisplay() {
  ...
  tray.setToolTip(`Personal Monitor - ${tip}`);
...
async function writeExportTo(dir, periods, options = {}) {
  ...
  const files = exportFileSet({
    periods: periods || {},
    history,
    meta: { generatedAt: new Date().toISOString(), app: { name: 'personal-monitor', version: appVersion() } }
  });
...
function isAllowedExternalUrl(value) {
  ...
  if (parsed.hostname === 'github.com' && parsed.pathname.startsWith('/Javis603/personal-monitor')) return true;
  if (
    (parsed.hostname === 'javis-ai.com' || parsed.hostname === 'www.javis-ai.com')
    && (parsed.pathname === '/personal-monitor' || parsed.pathname.startsWith('/personal-monitor/'))
  ) return true;
