...
const TOKEN_MONITOR_USER_AGENT = `personal-monitor/${appVersion()} (+https://github.com/Javis603/personal-monitor)`;
...
function fetchClaudeWebJson(url, headers, deps = {}, options = {}) {
  ...
  // `personal-monitor/<version>` agent with `403 cf-mitigated: challenge`, so that
...
async function runClaudePtyProbe(slashCommand, exitMarkerRegex, deps = {}) {
  ...
  const probeDir = deps.claudeProbeDir || path.join(os.tmpdir(), 'personal-monitor-claude-probe');
...
async function readCodexRpcWithCommand(command, deps = {}) {
  ...
  try {
    ...
    await rpc.send('initialize', {
      clientInfo: { name: 'personal-monitor', title: 'Personal Monitor', version: appVersion() }
    });
