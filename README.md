...
<div align="center">
    <img src=".github/assets/app.png" alt="Personal Monitor logo" width="120">
...
<p align="center">
    <a href="https://github.com/Javis603/personal-monitor/releases"><img src="https://img.shields.io/github/v/release/Javis603/personal-monitor?include_prereleases&style=flat-square&label=release&color=22c55e" alt="Latest release" /></a>
    <a href="https://github.com/Javis603/personal-monitor/releases"><img src="https://img.shields.io/github/downloads/Javis603/personal-monitor/total?style=flat-square&color=22c55e" alt="Total downloads" /></a>
...
Personal Monitor supports token usage, account-limit checks, and session details separately:
...
## Why Personal Monitor?

Most usage monitors are useful on the machine they run on. Personal Monitor is built for multi-device work: each device watches its own local logs, sends summary updates to your hub, and every connected widget sees token changes almost immediately.
...
- **Preserve deleted session usage** — many tools prune old sessions (Claude Code drops transcripts after 30 days by default), losing that history. When enabled, Personal Monitor archives observed daily tool/model usage locally so the heatmap and trends survive even after the source files are gone (see [Session data retention](#session-data-retention) below)
...
Download from [GitHub Releases](https://github.com/Javis603/personal-monitor/releases).
...
The hub runs while Personal Monitor is running — quitting (not just closing the window) stops it for all connected devices.
...
[![Deploy to Cloudflare](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/Javis603/personal-monitor/tree/main/worker)
...
| Platform | Path |
|----------|------|
| macOS | `~/Library/Application Support/Personal Monitor/` |
| Windows | `%APPDATA%/Personal Monitor/` |
| Linux | `~/.config/Personal Monitor/` |
...
With **Preserve deleted session usage** enabled (Settings → Collection), Personal Monitor archives observed daily tool/model usage locally with no time limit — so even after a source tool prunes its own sessions, the heatmap and trends are unaffected.
...
This archive only covers days Personal Monitor has already observed; data deleted before it started tracking cannot be recovered.

</details>

## Settings

There are two places to configure Personal Monitor; day-to-day use only needs the first:
